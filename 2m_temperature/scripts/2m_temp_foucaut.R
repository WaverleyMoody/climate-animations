...
# ── Paths ────────────────────────────────────────────────────────────────
data_file  <- "/Users/waverleymoody/Downloads/climate_data_by_variable/2m_temp_climatology.nc"
output_dir <- "/Users/waverleymoody/Downloads/2m_temp_animation_R"
...
# ── Plot settings (identical to the PlateCarree/Robinson script) ──────────
vmin <- -60
vmax <- 50
color_stops <- c(
...
n_colors <- 60
temp_palette <- colorRampPalette(color_stops)(n_colors)
...
# is what produced the dense internal hatching seen in the first test
# render. We only want that level of detail for the US.
...
r_stack <- rast(data_file, subds = "t2m")
r_stack <- rotate(r_stack)              # ERA5 is 0-360 -- fix before reprojecting
r_stack <- r_stack - 273.15             # Kelvin -> Celsius
...
# producing the striped background outside the continents. Cartopy avoided
# this in Python via an explicit boundary the GeoAxes clips to -- this
# replicates that by tracing the same up-the-antimeridian-and-back outline
# and masking the raster to it after projecting.
...
for (i in seq_len(n_frames)) {
  ...
  # ── Map panel ──
  # bty = "n" reliably suppresses the plot-region box; the box=FALSE arg
  # used previously isn't a real terra::plot() parameter and was being
  # silently dropped, which is why the black rectangle was still showing.
  par(mar = c(2, 3, 7.5, 2), bty = "n")
  plot(field_p, col = temp_palette, breaks = breaks, range = c(vmin, vmax),
  ...
  # not panel-local ones, so smallplot/horiz don't actually respect this
  # layout panel -- that's what was pinning it to a tall vertical strip on
  # the right regardless of the settings passed. Drawing it directly with
  # image() gives full control and keeps it inside this panel.
  ...
  pad <- (vmax - vmin) * 0.25
  image(x = seq(vmin, vmax, length.out = n_colors), y = 1, z = cbar_mat,
        col = temp_palette, breaks = breaks,
  ...
  mtext("Temperature at 2 m (\u00b0C)", side = 1, line = 3.0, cex = 1.0)
...
video_path <- file.path(output_dir, "2m_temperature_climatology_foucaut.mp4")
