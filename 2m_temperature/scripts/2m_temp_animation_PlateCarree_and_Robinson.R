...
#
# R translation of animate_2m_temp.py
# Generates the 2m temperature climatology animation from ERA5 reanalysis
# data (1979-2000), rendered across multiple map projections.
...
# Same 16 color stops as the Python custom_temp colormap, interpolated to 60
color_stops <- c(
  '#2d004b', # dark purple   (-60C)
  '#800080', # purple
  '#8b008b', # magenta
  '#ff00ff', # bright pink
  '#0000cd', # blue
...
r_stack <- rast(data_file, subds = "t2m") - 273.15   # Kelvin -> Celsius
...
for (proj_name in names(projections)) {
  ...
  # Reproject basemap + graticule layers once per projection (not per frame)
  ...
  for (i in seq_len(n_frames)) {
    ...
    for (ll in lat_labels) {
      ...
      off <- if (abs(lat_val) == 90 && proj_name == "robinson") 1.0 else 0.4
    ...
    par(mar = c(3, 6, 0.5, 6))
    n_swatches <- length(temp_palette)
    plot(NA, xlim = c(0, n_swatches), ylim = c(0, 1), axes = FALSE,
         xlab = "", ylab = "", xaxs = "i", yaxs = "i")
    rect(0:(n_swatches - 1), 0, 1:n_swatches, 1, col = temp_palette, border = NA)
    box(col = "black", lwd = 1)
    ...
    axis(1, at = tick_pos, labels = tick_vals, cex.axis = 0.8, tck = -0.3,
         mgp = c(3, 0.5, 0))
    mtext("Temperature at 2 m (\u00b0C)", side = 1, line = 2, cex = 0.9)
  ...
  video_name <- if (proj_name == "platecarree") {
    "2m_temperature_climatology.mp4"
  } else {
    sprintf("2m_temperature_climatology_%s.mp4", proj_name)
  }
