...
# ---- config ------------------------------------------------------------
NC_PATH = "/Users/waverleymoody/Downloads/climate_data_by_variable/slp_climatology.nc"
OUT_MP4_PATH = "/Users/waverleymoody/Desktop/nicolosi_slp.mp4"
WEST_LON = -90.0
EAST_LON = 90.0
FPS = 4
DPI = 130

# Same fixed color scale and custom colormap as animate_SLP.py, so this
...
def main():
    ...
    var_name = find_pressure_var(ds)
    print(f"Using variable: '{var_name}'")

    da = ds[var_name] / 100  # Pa -> hPa/mb, matching animate_SLP.py
    ...
    # VMIN/VMAX are fixed (matching animate_SLP.py's 970/1040 scale), not
    ...
    print(f"Actual data range: {actual_min:.1f} to {actual_max:.1f} mb "
    ...
    def render_frame(i):
        ...
        cbar.set_label('Pressure at Mean Sea Level (mb)', fontsize=14)
        cbar.ax.tick_params(labelsize=12)
        cbar.set_ticks(np.arange(970, 1041, 10))
        cbar.set_ticklabels([str(t) for t in np.arange(970, 1041, 10)])

        # Divider lines between the colormap's 60 discrete bands -- present
        # in animate_SLP.py but not animate_2m_temp.py, so replicated here
        # to match that script exactly rather than the temp version.
