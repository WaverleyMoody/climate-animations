...
#     starts 2000-02-18, 454 frames total. Uses PROJ's plain `fouc` (NOT
#     `fouc_s`/Foucaut Sinusoidal), which has a working inverse. Every map pixel
#     is inverse-projected from Foucaut to lon/lat ONCE with PROJ (sf_project) and
#     then checked by a forward round trip, so pixels outside the pointed-pole
#     outline (which PROJ would otherwise wrap back onto the globe) are dropped.
#     Since the projection never changes, each frame is then just a lookup into
#     that month's grid -- this also avoids the pcolormesh wrap/pole-sliver
#     striping the Python version works around by pre-projecting. Each frame
...
OUTPUT_PATH <- "/Volumes/CLIMATEDATA/gpp_2000_2009_foucaut_R.mp4"
FRAMES_DIR  <- "/Volumes/CLIMATEDATA/_frames_tmp_gpp_foucaut_R"
VAR_NAME    <- "gpp"

PROJ_CRS   <- "+proj=fouc +lon_0=0 +datum=WGS84 +units=m +no_defs"  # = the Python Foucaut class
...
# as the Python MAP_RECT. Like Cartopy, the map keeps Foucaut's true aspect
...
# The Foucaut outline: up the -180 meridian, back down the +180 meridian
# (the poles are points, so no pole lines are needed). Same trace as the
# Python Foucaut class, in projected meters.
.boundary_xy <- function() {
  lats <- seq(-90, 90, length.out = 200)
  p <- .forward(c(rep(-180, 200), rep(180, 200)), c(lats, rev(lats)))
  p[is.finite(p[, 1]) & is.finite(p[, 2]), , drop = FALSE]
}

# Map box in figure fractions and pixels, sized from the traced boundary's
# extent (not a hardcoded guess), as in the Python version.
.map_layout <- function() {
  b <- .boundary_xy()
  x_max <- max(abs(b[, 1]))
  y_max <- max(abs(b[, 2]))
...
# Inverse-projects every map pixel center to lon/lat. PROJ's fouc inverse does
# not reject points outside the outline -- it wraps their longitude back into
# +/-180 -- so each result is forward-projected again and kept only if it
# lands back on the same pixel (round-trip error well under one pixel).
.pixel_lonlat <- function(lay) {
  ...
  ok <- is.finite(ll[, 1]) & is.finite(ll[, 2]) &
    abs(ll[, 1]) <= 180 & abs(ll[, 2]) <= 90
  
  tol <- 0.1 * (2 * lay$x_max / lay$px_w)                          # 1/10 of a pixel
  back <- .forward(ll[ok, 1], ll[ok, 2])
  ok[ok] <- is.finite(back[, 1]) & is.finite(back[, 2]) &
    abs(back[, 1] - X[ok]) < tol & abs(back[, 2] - Y[ok]) < tol
  
  list(lon = ll[ok, 1], lat = ll[ok, 2], inside = ok)
}
...
# Foucaut outline (the map "spine") in figure fractions.
...
main <- function() {
  ...
  message("Precomputing Foucaut pixel lookup (one-time)...")
  lay <- .map_layout()
