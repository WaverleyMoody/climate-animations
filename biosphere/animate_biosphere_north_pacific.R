...
#
# Script: animate_biosphere_north_pacific.R
#
...
#     evolving; (2) a zoom into a fixed, close-up view of the focus region (currently the North
#     Pacific subtropical gyre, off the California/Baja coast) that replays the full 2000-2010
#     record a second time. Rendering is
...
#     animate_biosphere_north_pacific.py.
#     FRAMES_DIR/OUTPUT_PATH are specific to the current focus target -- changing FOCUS_LON/FOCUS_LAT
#     to a new region requires pointing these at a new, empty directory too, or the resume logic will
#     incorrectly reuse frames rendered for a different focus region.
...
FRAMES_DIR  <- "/Users/waverleymoody/CLIMATE ANIMATIONS/global_biosphere/frames_north_pacific_R"
OUTPUT_PATH <- "/Users/waverleymoody/CLIMATE ANIMATIONS/global_biosphere/animate_biosphere_north_pacific_R.mp4"
...
# Test mode: render only the first MAX_FRAMES frames and stitch them into a
# short "_test" MP4. Set to NULL for the full render.
MAX_FRAMES <- NULL
...
.assemble_video <- function(frames_dir, output_path, fps, n_frames = NULL) {
...
main <- function() {
  ...
  test_mode <- !is.null(MAX_FRAMES) && MAX_FRAMES < total_frames
  render_frames <- if (test_mode) MAX_FRAMES else total_frames
  if (test_mode) {
    message(sprintf("TEST MODE: rendering the first %d of %d frames.", render_frames, total_frames))
  }
  ...
  for (k in seq_len(render_frames)) {
  ...
  if (test_mode) {
    test_path <- sub("\\.mp4$", "_test.mp4", OUTPUT_PATH)
    message(sprintf("Test frames rendered to %s", FRAMES_DIR))
    .assemble_video(FRAMES_DIR, test_path, FPS, n_frames = render_frames)
