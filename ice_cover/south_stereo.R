...
#
# Script: south_stereo.R
# Description: Generates the Southern Hemisphere seasonal-cycle stereographic animation
# from NASA Blue Marble Next Generation monthly imagery (2004) with an ERA5 sea_ice_cover
# overlay (24 biweekly frames, 1st and 15th of each month), rendered in the South Polar
# Stereographic projection.
# Note: For the Northern Hemisphere, see north_stereo.R in the stereographic scripts folder.
...
FRAMES_DIR <- "/Volumes/CLIMATEDATA/stereographic/south/r_output/frames"
VIDEO_OUT <- "/Volumes/CLIMATEDATA/stereographic/south/r_output/south_seasonal_cycle_r.mp4"
...
SOUTH_CENTRAL_LON <- 300  # meridian pointing to the top of the map. Carried over from
# the tuned north value as a starting guess -- PROJ's stereo
# +lon_0 rotation for lat_0=-90 is not guaranteed to match the
# north script's visual orientation, so retune by eye.
FPS <- 6

CRS_SOUTH <- sprintf("+proj=stere +lat_0=-90 +lon_0=%d +datum=WGS84 +units=m +no_defs",
                     SOUTH_CENTRAL_LON)
...
load_month_image <- function(month) {
  ...
  # Flip applied AFTER ext/crs assignment, not before -- doing it before was
  # confirmed (in the north script) to have no effect, apparently because
  # reassigning ext() on a still-lazy raster discarded the pending flip.
...
hemisphere_crop <- function(r) {
  terra::crop(r, terra::ext(-180, 180, -90, SOUTH_MAX_LAT))
}
...
render_frame <- function(target_date, base_img, ice_layer, output_path) {
  ...
  base_proj <- terra::project(base_cropped, TEMPLATE_SOUTH, method = "bilinear")
  ice_proj <- terra::project(ice_cropped, TEMPLATE_SOUTH, method = "near")
  ...
  title(main = "Southern Hemisphere Seasonal Cycle", cex.main = 1.6, line = 3.2)
...
main <- function() {
  ...
  for (i in seq_along(TARGET_DATES)) {
    ...
    out_path <- file.path(FRAMES_DIR, sprintf("south_%s.png", format(target_date, "%Y-%m-%d")))
