...
overlay (24 biweekly frames, 1st and 15th of each month), rendered in the North Polar Stereographic 
projection.
Note: For the Southern Hemisphere, see south_stereo.py in the stereographic scripts folder.
"""
...
FRAMES_DIR = Path("/Volumes/CLIMATEDATA/stereographic/north/frames")
VIDEO_OUT = Path("/Volumes/CLIMATEDATA/stereographic/north/north_seasonal_cycle.mp4")
...
NORTH_CENTRAL_LON = 300  # meridian pointing to the top of the map; 180 puts Europe
                          # near the top and the Americas near the bottom (they aren't
                          # exactly antipodal, so nudge this if you want it tighter)
FPS = 6
...
def render_frame(target_date: dt.date, base_img: np.ndarray,
    ...
    proj = ccrs.NorthPolarStereo(central_longitude=NORTH_CENTRAL_LON)
    # Fixed, symmetric rect (left, bottom, width, height) in figure fraction --
    # equal left/right margins keep the globe horizontally centered, and using
    # an explicit rect (rather than default placement + a tight-crop save)
    # guarantees every frame renders at the same size and position.
    ax = fig.add_axes([0.05, 0.09, 0.90, 0.82], projection=proj)
    ax.set_facecolor("white")

    ax.set_extent([-180, 180, NORTH_MIN_LAT, 90], ccrs.PlateCarree())
    ...
    fig.suptitle("Northern Hemisphere Seasonal Cycle", fontsize=16, y=0.97, x=0.5, ha="center")
...
def main():
    ...
    for target_date in TARGET_DATES:
        ...
        out_path = FRAMES_DIR / f"north_{target_date.isoformat()}.png"
