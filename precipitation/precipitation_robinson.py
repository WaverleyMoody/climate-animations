...
# ERA5 stores longitude as 0-360; normalize to -180-180 and sort so Cartopy
# doesn't draw a seam/duplicate wrap at the prime meridian.
...
# ── Assemble into MP4 ─────────────────────────────────────────────────────────
with imageio.get_writer(OUTPUT_DIR / 'precipitation_climatology_robinson.mp4',
                         fps=4, codec='libx264') as writer:
    for fp in frame_paths:
        writer.append_data(imageio.imread(fp))

print('Done! Animation saved to:', OUTPUT_DIR / 'precipitation_climatology_robinson.mp4')