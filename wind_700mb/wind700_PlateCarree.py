...
rendered in the Plate Carrée projection.

Note: For the Robinson, Foucaut, and Nicolosi projections, see the other
scripts in the wind_700mb scripts folder.
"""
...
# ── Plot settings ─────────────────────────────────────────────────────────────
# Colorbar capped at 30 m/s to bring out mid-latitude jet stream detail.
# The full 0–60 range compressed everything into a narrow green band with
# little visible contrast.
VMIN, VMAX = 0, 30
...
for i in range(ds_clim.sizes['frame']):
    # squeeze() removes the pressure_level dimension (size 1) if present.
...
video_name = 'wind700_climatology.mp4'
