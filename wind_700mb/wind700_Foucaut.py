...
class Foucaut(ccrs.Projection):
    ...
    Confirmed working via wind_speed_foucaut.py."""
...
for i, (label, mean_speed, frame_u, frame_v) in enumerate(frames_data):
    ...
    # so subsampling needs no separate re-sort.
    step = 30
    lons_sub = lons[::step]
    lats_sub = lats[::step]
    ...
    # Foucaut has a working inverse transform, so Cartopy handles wind
    # vector rotation automatically -- same as Robinson.
    ax.quiver(
        lons_grid, lats_grid,
        u_sub, v_sub,
        transform=ccrs.PlateCarree(),
    ...
    # Single pole labels (the whole pole is one point in this projection).
    ...
    # Latitude labels along the left edge, excluding the poles.
...
video_name = 'wind700_climatology_foucaut.mp4'
