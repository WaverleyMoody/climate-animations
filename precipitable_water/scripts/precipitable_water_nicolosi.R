...
data_file <- paste0(
  ...
  "climate_data_by_variable/precip_water_climatology.nc"
)
...
# Same fixed color scale and custom colormap as precipitable_water_
# platecarree.R / precipitable_water_robinson.R, so this animation
# matches the PlateCarree/Robinson ones on the site.
vmin <- 0
vmax <- 80
...
# Convert ERA5 longitudes from 0–360 to -180–180. No unit conversion --
# tcwv is used directly in kg/m^2, matching animate_nicolosi_pw.py.
...
for (i in seq_len(n_frames)) {
  ...
  # (SLP_nicolosi.R uses it, the Python animate_nicolosi_pw.py source
  # this script is translated from does not), same as the precipitable
  # water PlateCarree/Robinson R scripts, which also have no colorbar
  # dividers.
