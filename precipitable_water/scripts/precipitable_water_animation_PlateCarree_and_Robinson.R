...
#
# R translation of animate_precipitable_water.py
# Generates the precipitable water (total column water vapour) climatology
# animation from ERA5 reanalysis data (1979-2000), rendered across multiple
# map projections.
...
for (proj_name in names(projections)) {
  ...
  video_name <- if (proj_name == "platecarree") {
    "precipitable_water_climatology.mp4"
  } else {
    sprintf("precipitable_water_climatology_%s.mp4", proj_name)
  }
