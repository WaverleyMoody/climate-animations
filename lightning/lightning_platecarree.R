...
#
# Script: lightning_platecarree.R
# Description: Generates the lightning climatology animation from WWLLN/WGLC data (2010-2025), rendered in the Plate Carrée projection. 365 calendar-day frames (Feb 29 excluded), each the multi-year daily-climatological mean, annualized (x365) to match the reference product's strokes km-2 yr-1 units.
#
# Note: For the Robinson, Foucaut, and Nicolosi projections, see the other scripts in the lightning scripts folder.
...
# the log10 range from vmin to vmax -- this is the R equivalent of the
# Python script's log_pos() helper, used identically for both the palette
# color-stop positions and the colorbar tick positions.
...
# Log-spaced breaks (not linear) -- this is the direct R equivalent of the
# Python script's np.logspace()-based log_levels/LogNorm. Each break is
...
for (i in seq_len(n_frames)) {
  ...
  field_p <- clamp(field, lower = vmin, upper = vmax, values = TRUE)
  ...
  for (ll in lat_labels) {
    ...
    text(xy[1], xy[2], labels = lab, pos = 2, cex = 0.75, xpd = NA, offset = 0.4)
  }
  ...
  # formula the temperature script uses -- the swatches themselves are laid
  # out evenly in log-space (see make_custom_palette's eval_pos), so tick
  # placement has to follow the same log-space fraction to land correctly.
...
video_name <- "lightning_climatology.mp4"
